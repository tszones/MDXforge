...
function createMdxFolderEntry(file: PageFile): MdxFolderEntry {
...
function attachDocumentLinks(rootPath: string, entries: MdxFolderEntry[]): void {
  const index = createDocumentIndex(entries)
  const entriesByPath = new Map(entries.map((entry) => [entry.path, entry]))

  for (const entry of entries) {
    entry.links = extractDocumentLinks(rootPath, entry, index)
  }
