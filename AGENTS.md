...
AI 写代码时优先遵守：
1. 本地优先：不要引入云端依赖；默认读取本地文件。
2. 安全渲染：MDX 组件走白名单；谨慎扩大可执行/可注入能力。
3. 跨平台：Windows/macOS/Linux 都要可用。
4. 多语言：新增 UI 文案必须中英双语。
5. 类型安全：保持 TypeScript 类型完整。
6. 小步实现：优先改清晰边界内的文件。
7. 文档体验：UI 应像文档站，阅读、导航、目录优先。
