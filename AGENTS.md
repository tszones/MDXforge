...
这是一个 pnpm workspace / Turborepo monorepo。当前唯一应用是 Electron + React 桌面应用：
- `apps/desktop`：MDXForge 桌面端。
- `packages/ui`：桌面端与未来官网共享的 React UI primitives。
- `packages/*`：后续其它共享包。
- 未来官网应放在 `apps/web`，不要和桌面端代码混在一起。
...
关键路径：
- `apps/desktop/src/main/index.ts`：Electron app 启动、窗口、IPC、打开路径、单实例处理。
- `apps/desktop/src/main/mdx.ts`：MDX / Markdown 文件读取、frontmatter 解析、MDX 编译、最近打开路径。
- `apps/desktop/src/main/page-tree.ts`：扫描文件夹、生成文档树、支持 `meta.json` 排序/分组/外链。
- `apps/desktop/src/main/mdx-options.ts`：MDX 插件配置，支持 Mermaid、数学公式、代码高亮、Twoslash、TOC。
- `apps/desktop/src/renderer/src/App.tsx`：应用主界面，打开文件/文件夹、设置页、预览页切换。
- `apps/desktop/src/renderer/src/components/MdxPreview.tsx`：MDX 渲染、侧边栏、TOC、文档切换。
- `apps/desktop/src/renderer/src/components/mdx-components.tsx`：允许 MDX 使用的 Fumadocs 组件白名单。
- `apps/desktop/messages/en-US.json` / `apps/desktop/messages/zh-CN.json`：多语言文案源。
...
- 应用 UI 文案从 `apps/desktop/messages/*.json` 管理。
- 运行时通过 `apps/desktop/src/renderer/src/lib/language.ts` 应用语言。
- 设置页允许切换界面语言。
- 注意：语言切换只影响应用 UI，不自动翻译 MDX 文档内容。

新增/修改 UI 文案时：
- 必须同时更新 `apps/desktop/messages/en-US.json` 和 `apps/desktop/messages/zh-CN.json`。
...
Electron Builder 配置在 `apps/desktop/electron-builder.yml`：
- Windows：`mdx`、`md` 文件关联。
- macOS：DMG 构建、基础权限描述。
...
- 改主进程能力：看 `apps/desktop/src/main/*`。
- 改 renderer UI：看 `apps/desktop/src/renderer/src/*`。
- 改可用 MDX 组件：看 `apps/desktop/src/renderer/src/components/mdx-components.tsx`。
- 改 MDX 编译插件：看 `apps/desktop/src/main/mdx-options.ts`。
- 改文档树/文件夹扫描：看 `apps/desktop/src/main/page-tree.ts`。
- 改应用设置：看 `apps/desktop/src/main/settings.ts`、`SettingsPage.tsx`。
- 改多语言：同步改 `apps/desktop/messages/en-US.json` 和 `apps/desktop/messages/zh-CN.json`。
